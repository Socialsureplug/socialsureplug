...
@section('content')
    <div class="container">
        <div class="container-header">
            ...
            <div class="container-right">
                ...
                <button type="button" class="btn btn-primary" id="update-renting-countries">
                    <i class="las la-sync"></i>Update Country
                </button>
                <button class="btn btn-primary" id="add-renting-country"><i class="las la-plus"></i>Add Country</button>
            </div>
        </div>

        <div class="card table-card">
            <div class="filter-grid">
                <form method="GET" action="{{ route('backend.rentings.api.countries', $api->id) }}" class="table-search-form">
                    ...
                    @if (!empty($search))
                        <a href="{{ route('backend.rentings.api.countries', $api->id) }}" class="btn btn-outline clear-btn">Clear</a>
                    @endif
                </form>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="table-actions">
                                        <a href="{{ route('backend.rentings.api.countries.operators', [$api->id, $country->id]) }}"
                                            class="table-btn" title="Operators">
                                            <i class="las la-sitemap"></i>
                                        </a>
                                        <button type="button" class="table-btn edit-renting-country"
                                        ...
                                        <form action="{{ route('backend.rentings.countries.destroy', $country->id) }}"
                                            method="post" class="d-inline" data-confirm="Delete this country?">
                                            @csrf
                                            @method('DELETE')
    ...
    <div class="modal" id="add-renting-country-modal">
        <div class="modal-content">
            ...
            <form action="{{ route('backend.rentings.api.countries.store', $api->id) }}" method="POST">
                @csrf
                <div class="modal-body">
                    <div class="form-group mb-25">
                        <label class="form-label">Name<span class="required">*</span></label>
                        <input type="text" class="form-control" name="name" placeholder="e.g. Kazakhstan" required>
                    </div>
                    <div class="form-group mb-25">
                        <label class="form-label">Code<span class="required">*</span></label>
                        <input type="text" class="form-control" name="code" placeholder="e.g. KZ" required>
                    </div>
                    <div class="form-group mb-25">
                        <label class="form-label">Country ID<span class="required">*</span></label>
                        <input type="text" class="form-control" name="country_id" placeholder="e.g. KZ" required>
                    </div>
                    <div class="form-group mb-25">
    ...
    <div class="modal" id="edit-renting-country-modal">
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">Edit Country</h3>
                <button type="button" class="close-modal"><i class="las la-times"></i></button>
            </div>
            <form action="{{ route('backend.rentings.countries.update', ['id' => '__ID__']) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="form-group mb-25">
...
@push('script')
<script>
document.getElementById('add-renting-country')?.addEventListener('click', function() {
    document.getElementById('add-renting-country-modal')?.classList.add('active');
});

document.addEventListener('DOMContentLoaded', function() {
    const updateBtn = document.getElementById('update-renting-countries');
    ...
    if (updateBtn) {
        updateBtn.addEventListener('click', async function() {
            ...
            try {
                const response = await fetch('{{ route('backend.rentings.api.countries.import', $api->id) }}', {
                    method: 'POST',
                    headers: {
    ...
    document.querySelectorAll('input[type="checkbox"][id^="toggle-"]').forEach(function(toggle) {
        toggle.addEventListener('change', function() {
            ...
            fetch('{{ route('backend.rentings.countries.toggle-status') }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Accept': 'application/json',
            ...
            .then(function(data) {
                if (data.success) {
                    iziToast.success({ message: data.message || 'Status updated successfully', position: 'topRight' });
                } else {
    ...
    document.querySelectorAll('.edit-renting-country').forEach(function(btn) {
        btn.addEventListener('click', function() {
            const modal = document.getElementById('edit-renting-country-modal');
            ...
            modal.querySelector('form').action = '{{ route("backend.rentings.countries.update", ["id" => "__ID__"]) }}'.replace('__ID__', this.getAttribute('data-id'));
